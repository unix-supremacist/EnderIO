...
public class ItemSoulCrusher extends ItemTool implements IEnergyTool, IEqualityOverrideItem, IEmpowerableItem {
            ...
            switch (getMode(stack)) {
                case 1:
                    if (BlockHelper.getCurrentMousedOverSide(player) == 0
                            || BlockHelper.getCurrentMousedOverSide(player)
                                    == 1) { // if looking at the top or bottom of a block
                        switch (MathHelper.floor_double((player.rotationYaw * 4F) / 360F + 0.5D)
                                & 3) { // Stolen from MineFactoryReloaded
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean wtf) {
        ...
        if (MiscUtil.isShiftPressed()) {
            for (Map.Entry<AbstractSoulUpgrade, Byte> entry :
                    SoulUpgradeHelper.getUpgrades(stack).entrySet()) { // installed upgrades
                list.add(EnumChatFormatting.DARK_AQUA.toString()
                        + StringHelper.writeUpgradeInfo(entry.getValue(), entry.getKey()));
            ...
            for (AbstractSoulUpgrade upgrade : SoulUpgrades.registry) { // available recipes
                if (upgrade.isUpgradeAvailable(stack)) {
                    ...
                    list.add(EnumChatFormatting.DARK_AQUA.toString()
                            + StringHelper.writeUpgradeInfo(
                                    SoulUpgradeHelper.getUpgradeLevel(stack, upgrade) + 1, upgrade));
    ...
    public void registerIcons(IIconRegister register) {
        itemIcon = register.registerIcon(EnderIO.MODID.toLowerCase() + ":" + Names.SOUL_CRUSHER);
    }

    @Override
    public String getUnlocalizedName() {
        return "item." + EnderIO.MODID.toLowerCase() + ":" + Names.SOUL_CRUSHER;
    }
    ...
    public int getMode(ItemStack stack) {
        ...
        if (stack.stackTagCompound.hasKey("Mode")) return stack.stackTagCompound.getByte("Mode");
        else return 0;
    }
    ...
    public boolean setEmpoweredState(ItemStack stack, boolean b) {
        ...
        if (getMode(stack) == getNumModes(stack)) setMode(stack, 0);
