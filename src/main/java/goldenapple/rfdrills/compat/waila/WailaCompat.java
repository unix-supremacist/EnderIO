...
public class WailaCompat implements IWailaDataProvider {
    ...
    public List<String> getWailaHead(
            ItemStack itemStack, List<String> tooltip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        return null;
    }
    ...
    public List<String> getWailaBody(
        ...
        if (accessor.getPlayer().getCurrentEquippedItem() == null
                || !(accessor.getPlayer().getCurrentEquippedItem().getItem() instanceof IEnergyTool)) return tooltip;
        ...
        if (equipStack.getItem() instanceof IEnergyTool) {
            if (config.getConfig("rfdrills.waila_rf")) {
                boolean harvest = ToolHelper.isToolEffective(equipStack, block, meta)
                        || block.getBlockHardness(accessor.getWorld(), pos.blockX, pos.blockY, pos.blockZ) == 0;
                tooltip.add(StringHelper.writeEnergyPerBlockInfo(
                        energyTool.getEnergyPerUse(equipStack, block, meta), harvest));
            }
            if (config.getConfig("rfdrills.waila_mode")
                    && (accessor.getPlayer().isSneaking() || !(config.getConfig("rfdrills.waila_mode.sneakingonly")))) {
    ...
    public static void init(IWailaRegistrar registrar) {
        registrar.addConfig(EnderIO.MOD_NAME, "rfdrills.waila_rf", "option.rfdrills.waila_rf");
        registrar.addConfig(EnderIO.MOD_NAME, "rfdrills.waila_mode", "option.rfdrills.waila_mode");
        registrar.addConfig(
                EnderIO.MOD_NAME, "rfdrills.waila_mode.sneakingonly", "option.rfdrills.waila_mode.sneakingonly");
