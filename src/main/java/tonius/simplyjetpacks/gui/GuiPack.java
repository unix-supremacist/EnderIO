...
public class GuiPack extends GuiBase {
    ...
    public GuiPack(ContainerPack container) {
        ...
        String unlocalizedTitle = "item." + SimplyJetpacks.PREFIX + container.pack.getBaseName(false)
                + container.packItem.modType.suffix + ".name";
    ...
    public void initGui() {
        ...
        if (this.container.energyStored != null) {
            ...
            this.addElement(new ElementFluidTankAdv(this, 12, 24, this.container.fluidStored)
                    .setAlwaysShow(true)
                    .setGauge(1));
        }
        ...
        for (int i = 0; i < controls.length; i++) {
            ...
            button.setToolTip(SJStringHelper.localize(
                            String.format("%s.button.%s", this.container.pack.getGuiTitlePrefix(), key.name)))
