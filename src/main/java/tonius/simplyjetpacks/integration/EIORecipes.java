...
public abstract class EIORecipes {
    ...
    private static void appendItemStack(StringBuilder sb, ItemStack stack) {
        if (stack != null) {
            String[] itemName =
                    Item.itemRegistry.getNameForObject(stack.getItem()).split(":");
            sb.append("<itemStack modID=\"" + itemName[0] + "\" itemName=\"" + itemName[1] + "\" itemMeta=\""
                    + stack.getItemDamage() + "\" number=\"" + stack.stackSize + "\" />");
        }
    }

    public static void addSoulBinderRecipe(
            String recipeID, int energy, int xp, String soulTypes, ItemStack input, ItemStack output) {
