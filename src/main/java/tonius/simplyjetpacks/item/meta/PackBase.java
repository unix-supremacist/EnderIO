...
public class PackBase {
        ...
        if (this.fuelType == FuelType.ENERGY) {
            for (int i = 0; i <= 4; i++) {
                ...
                if (currentStack != null
                        && currentStack != stack
                        && currentStack.getItem() instanceof IEnergyContainerItem) {
                    IEnergyContainerItem heldEnergyItem = (IEnergyContainerItem) currentStack.getItem();
                    if (this.usesFuel) {
                        int energyToAdd = Math.min(
                                item.useFuel(stack, this.fuelPerTickOut, true),
                                heldEnergyItem.receiveEnergy(currentStack, this.fuelPerTickOut, true));
    ...
    protected void toggleState(
            boolean on, ItemStack stack, String type, String tag, EntityPlayer player, boolean showInChat) {
        ...
        if (player != null && showInChat) {
            ...
            type = type != null && !type.equals("")
                    ? "chat." + this.name + "." + type + ".on"
                    : "chat." + this.name + ".on";
            String msg = SJStringHelper.localize(type) + " " + color
                    + SJStringHelper.localize("chat." + (on ? "disabled" : "enabled"));
    ...
    public String getArmorTexture(ItemStack stack, Entity entity, int slot, ModType modType) {
        ...
        return SimplyJetpacks.RESOURCE_PREFIX + "textures/armor/" + this.getBaseName(true) + modType.suffix + flat
                + ".png";
    }
    ...
    public void addInformation(ItemStack stack, ItemPack item, EntityPlayer player, List list) {
        ...
        list.add(SJStringHelper.getFuelText(
                this.fuelType, item.getFuelStored(stack), this.fuelCapacity, !this.usesFuel));
    }
    ...
    protected void loadConfig(Configuration config) {
        ...
        if (this.defaults.fuelPerTickOut != null) {
            this.fuelPerTickOut = config.get(
                    ...
                    .setMinValue(0)
                    .getInt(this.defaults.fuelPerTickOut);
        }
        if (this.defaults.armorReduction != null) {
            this.armorReduction = config.get(
                            this.defaults.section.name,
                            "Armor Reduction",
                            this.defaults.armorReduction,
        ...
        if (this.defaults.armorFuelPerHit != null) {
            this.armorFuelPerHit = config.get(
                            this.defaults.section.name,
                            "Armor Fuel Per Hit",
                            this.defaults.armorFuelPerHit,
                            "How much fuel is lost from this pack when the user is hit, if armored.")
                    .setMinValue(0)
                    .getInt(this.defaults.armorFuelPerHit);
        }
        if (this.defaults.enchantability != null) {
            this.enchantability = config.get(
                            this.defaults.section.name,
                            "Enchantability",
                            this.defaults.enchantability,
                            "The enchantability of this pack. If set to 0, no enchantments can be applied.")
                    .setMinValue(0)
                    .getInt(this.defaults.enchantability);
        }
    }
