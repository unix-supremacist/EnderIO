...
public abstract class SJStringHelper {
    ...
    public static String getFuelText(FuelType fuelType, int amount, int max, boolean infinite) {
        if (infinite) {
            return StringHelper.LIGHT_GRAY
                    + localize("tooltip.fuel.infinite." + fuelType.toString().toLowerCase());
        }
    ...
    public static String getStateText(boolean state) {
        String onOrOff = state
                ? StringHelper.BRIGHT_GREEN + localize("tooltip.on")
                : StringHelper.LIGHT_RED + localize("tooltip.off");
        return StringHelper.ORANGE + localize("tooltip.state") + ": " + onOrOff;
    }

    public static String getHoverModeText(boolean state) {
        String enabledOrDisabled = state
                ? StringHelper.BRIGHT_GREEN + localize("tooltip.enabled")
                : StringHelper.LIGHT_RED + localize("tooltip.disabled");
    ...
    public static String getEnergySendText(int send) {
        return StringHelper.ORANGE + localize("tooltip.energySend") + ": " + StringHelper.LIGHT_GRAY
                + getFormattedNumber(send) + " RF/t";
    }

    public static String getEnergyReceiveText(int receive) {
        String usageText =
                receive < Integer.MAX_VALUE ? getFormattedNumber(receive) + " RF/t" : localize("tooltip.energy.none");
    ...
    public static String getParticlesText(ParticleType particle) {
        return StringHelper.ORANGE + localize("tooltip.particles") + ": " + StringHelper.LIGHT_GRAY
                + localize("tooltip.particles." + particle.ordinal());
    }
    ...
    public static String getHUDStateText(Boolean engine, Boolean hover, Boolean charger) {
        String text = "";
        if (engine != null) {
            text += (engine ? StringHelper.BRIGHT_GREEN : StringHelper.RED)
                    + localize("gui.hud.state.engine")
                    + StringHelper.END;
        }
        if (hover != null) {
            if (engine != null) {
            ...
            text += (hover ? StringHelper.BRIGHT_GREEN : StringHelper.RED)
                    + localize("gui.hud.state.hover")
                    + StringHelper.END;
        }
        if (charger != null) {
    ...
    public static String getShiftText() {
        return StringHelper.LIGHT_GRAY
                + String.format(
                        localize("tooltip.holdShift"),
                        StringHelper.YELLOW
                                + StringHelper.ITALIC
                                + localize("tooltip.holdShift.shift")
                                + StringHelper.END
